...
from mongo_models.opportunity_fields import OpportunityForm, ResponseData, FieldError
...
class OpportunityResponse(Base):
    ...
    def create(cls, session: Session, user: _user.User, opportunity: _opportunity.Opportunity,
               form: OpportunityForm, data: ser.OpportunityResponse.CreateFields) -> Self | list[FieldError]:
        response = OpportunityResponse(user=user, opportunity=opportunity)
        session.add(response)
        session.flush([response])
        saved_data = ResponseData.create(response_id=response.id, form=form, data=data)
        if not isinstance(saved_data, ResponseData):
            return saved_data
        return response
