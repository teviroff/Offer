...
class OpportunityResponse(Base):
    ...
    def create(cls, session: Session, fields: ser.OpportunityResponse.Create) \
        ...
        if user is None:
            logger.debug('\'OpportunityResponse.create\' exited with \'INVALID_USER_ID\' error (user_id=%i)',
                         fields.user_id)
            return GenericError(
        ...
        if opportunity is None:
            logger.debug('\'OpportunityResponse.create\' exited with \'INVALID_OPPORTUNITY_ID\' '
                         'error (opportunity_id=%i)', fields.opportunity_id)
            return GenericError(
