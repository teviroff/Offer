...
function transformCardsTags() {
    ...
    Array.from(cardsContainer.children).forEach((card) => {
        const cardTagsContainer = card.getElementsByClassName("card-tags-container")[0]
        const geotagsJSON = JSON.parse(cardTagsContainer.children[0].innerText)
        const tagsJSON = JSON.parse(cardTagsContainer.children[1].innerText)
        cardTagsContainer.innerText = ""
        geotagsJSON.forEach((geotag) => {
            cardTagsContainer.appendChild(createGeoTag(geotag["id"], geotag["name"]))
        })
