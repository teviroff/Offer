...
class CreateOpportunityCardFormatter(BaseSerializerFormatter, BaseDBFormatter):
    ...
    serializer_error_appender = APISerializerErrorAppender(
        ...
        title=append_serializer_field_error_factory(
            field_name='title',
            transformer=transform_str_error_factory('Opportunity card title', min_length=1, max_length=30)
        ),
        sub_title=append_serializer_field_error_factory(
            field_name='sub_title',
            transformer=transform_str_error_factory('Opportunity card subtitle', min_length=1, max_length=30)
        ),
    )
